...
local function create_mouse_binding(event, mods, action)
	return { event = event, mods = mods, action = action }
end

local config = {
	font = wezterm.font_with_fallback({ "TX02 Nerd Font" }),
	font_size = 16.0,
	...
	automatically_copy_selection = true,
}

config.mouse_bindings = {
	create_mouse_binding(
...
config.keys = {
	create_keybind("d", "CMD", act.SplitHorizontal({ domain = "CurrentPaneDomain" })),
	create_keybind("d", "CMD|SHIFT", act.SplitVertical({ domain = "CurrentPaneDomain" })),
	...
	create_keybind("x", "LEADER", act.CloseCurrentPane({ confirm = false })),
