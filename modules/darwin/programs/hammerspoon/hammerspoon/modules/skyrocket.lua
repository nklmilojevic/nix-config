-- Move/resize windows by holding modifiers and dragging anywhere in the window.
-- Adapted from the SkyRocket Spoon by David Balatero (MIT), with local
-- robustness fixes (drag watchdog, pcall on finish, stuck-drag recovery).
...
-- Kept global so the handlers/eventtaps aren't garbage collected.
sky = SkyRocket:new({
	opacity = 0.3,
